...
Future<void> initializeBackService() async {
  ...
  await service.configure(
      iosConfiguration: IosConfiguration(
        ...
        autoStart:
            true, // this will be executed when app is in foreground in separated isolate
      ),
      ...
      if (service is AndroidServiceInstance) {
        if (await service.isForegroundService()) {
          service.setForegroundNotificationInfo(
              title: "Flutter development", content: "Background service ");
          print("foreground service running ${timer.tick}");
        }
      }
      print("background service running ${timer.tick}");
      service.invoke("update", {
        "current_date": DateTime.now().toIso8601String() + "hello woeld",
      });
    });
  });
}
